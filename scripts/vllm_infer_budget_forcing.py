...
def yield_chunks(dataset, template_obj, prev_cot, reflect_text, tokenizer, image_resolution, chunk_size, resize_images: bool):
    chunk_prev_cot = []
    ...
    for sample in tqdm(dataset, desc="Preparing data"):
        ...
        #prev_cot = next(prev_cot)
        new_prompt = f"{prompt} {next(prev_cot)} {reflect_text}"
        prompts.append(new_prompt)
        inputs.append({"prompt_token_ids": tokenizer.encode(new_prompt), "multi_modal_data": multi_modal_data})
...
def vllm_infer(
    ...
    #prev_cot_path = "/h/andrewliao/research/visual_reasoning_pomdp/outputs/2025-3-5/10-52-30-765766/outputs/eval/checkpoint-138/outputs/high_res_datasets_generated_predictions.jsonl"
    import pandas as pd
    df = pd.read_json(prev_cot_path, lines=True)
    prev_cot = df["predict"].tolist()
    assert len(dataset_module["train_dataset"]) == len(prev_cot), "The number of samples in the dataset and the number of samples in the prev_cot file should be the same."
    prev_cot = [s[0] for s in prev_cot]             # pick the first predict for every sample
    prev_cot = [s.split("</think>")[0].strip() for s in prev_cot]
    prev_cot = islice(prev_cot, n_processed_samples, None)
    reflect_text = "Wait,"
    
    for inputs, prompts, labels in yield_chunks(dataset, template_obj, prev_cot, reflect_text, tokenizer, image_resolution, chunk_size, resize_images):
